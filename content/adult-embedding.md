...
`official`(79.89%) と `robust_numeric`(81.01%) の acc 差は約 1.1 点で標準偏差（0.58〜1.30）と
同程度で**有意ではない**。`adult_semantic`(77.78%) と `public_fe`(78.61%) は acc では official を
やや下回る（差 1〜2 点、ただし `adult_semantic` は std 1.84 と大きい）。一方 **macro F1** は
`adult_semantic` 62.58 ± 9.33（min 49.44 / max 70.09）・`public_fe` 63.28 ± 7.72（max 73.17）で
**seed 分散が 8〜9 点あり、分布は official（70.58 ± 1.18）と大きく重なる**（両者の max は official の
平均に達する／上回る）。したがって **macro F1 での variant 間順位は統計的に区別できず、「F1 が
悪化する」とは断定しない（n=3 では判定保留）**。この n=3 で確実に言えるのは、**capital fidelity は
明確に改善する一方、下流 acc は改善しない**という点に限られる。
...
- **収穫逓減し、ε≈8/∞ で頭打ち**（acc ~82.4%）。ノイズを完全に切っても（∞）それ以上は伸びない。
  ε=8 と ∞ の差は単一 seed の揺れの範囲（ε=∞ は 3 seed で 82.43 ± 0.03）。
- **ε=∞（ノイズなし）でも実データ上限に届かない**。PE 上限は 3 seed で測り直すと 82.43 ± 0.03
  （seed 間で極めて安定）で、同サイズ実データ 84.01% より ~1.6 点低い。ε=1 の official 埋め込みは
  3 seed 平均 79.89 ± 0.58 なので、上限との ~4.1 点差は、**DP ノイズ分（ε で埋まる）~2.5 点
  （= 82.43−79.89）**＋**PE 生成機構分（ε でも埋まらない）~1.6 点**に分解できる。ε=∞ の std（0.03）
  ≪ ε=1 の std（0.58）なので両者は分離できる。〔旧版は ε=1 に単一 seed の 80.67%（3 seed 中の最良
  draw）を使い DP ノイズ分を ~1.8 点と過小に見せていた。3 seed 平均でやり直すと 2.5 点に広がる。〕
...
official が上回る（上表は単一 seed のスイープ）。決め手の **ε=∞ は official/robust とも 3 seed で
測り直した**: official **82.43 ± 0.03**（seed 間で極めて安定）に対し robust **78.23 ± 1.69**
（seed 0/1/2 = 77.63 / 76.53 / 80.54 と不安定）。平均差は **−4.20**、かつ **official の最悪 seed
(82.39) が robust の最良 seed(80.54) を 1.85 点上回る**ため、「ε=∞ では official が明確に勝つ」は
単一 seed の −4.84 に依存せず 3 seed でも維持される。robust ε=∞ は acc こそ official に届かないが
macro F1 の seed 分散が大きい（F1 std 10.7、非単調）ため、ε 中間域の曲線の**形状**は引き続き
過信しない——安定して言えるのは「ε=∞ の acc で official が明確に上」までである。
...
- 埋め込みの精緻化は、**狙った列（capital）の分布忠実度は明確に改善する**が、**下流分類 acc は
  改善しない**。`robust_numeric` は誤差内の微増、`adult_semantic`/`public_fe` は acc をやや落とす。
  macro F1 は seed 分散が 8〜9 点と大きく、variant 間の優劣は n=3 では統計的に区別できない（判定保留）。
- ε は**レバーになる**が収穫逓減し、ノイズを切っても（ε=∞）PE 上限は 3 seed で 82.43 ± 0.03 に
  頭打ちで、同サイズ実データ上限 84% には届かない（残差 ~1.6 点は生成機構由来）。
- **埋め込み × ε**: robust の ε=1 での微増はノイズが埋め込みの粗さを覆った低 ε の産物で、ノイズを
  切ると official が明確に勝つ（ε=∞ を 3 seed で再測定: official 82.43 ± 0.03 > robust 78.23 ± 1.69、
  official の最悪 seed > robust の最良 seed）。DP ノイズは埋め込みの利点を
