...
![Adult utility vs ε](results/figures/adult_epsilon_sweep.png)

*図 7: Adult の acc/macroF1/AUC を ε で振った曲線（単一 seed）。破線は同サイズ実データの acc 上限
84.01%、点線は多数派ベースライン 75.77%。*
...
- **教訓**: Tab-PE の埋め込みは「分類器に効く良い特徴量」を目指すのではなく、**列の情報を粗くせず、
  ...
  スケールだけ直し、他は落とさない）が 4 variant 中で最良だったのはこのためと解釈できる。
