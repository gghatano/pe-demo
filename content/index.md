...
`9078c67` に固定して利用した。`uv` で環境を固定し、SCM の 3 prior（rff/tree/nn）と
実データ 4 種（Breast Cancer・Adult・Artificial Characters・Person Activity）を実行、
XOR は生成のみ実行した（分類器はライセンスの都合で `NOT_RUN`）。
