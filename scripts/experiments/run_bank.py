"""Bank Marketing Tab-PE run (issue #49).

Runs official Tab-PE on the locally-prepared Bank Marketing data
(`scripts/datasets/bank/preprocess.py` must have been run first). Uses the
official `TabularEmbedding`, seed control, and the same run config as the Adult
demo (30 iterations, 1000 samples, WSD 1/2/3-way, tabicl) so results are
comparable. Only the data and (optionally) epsilon/seed change; the algorithm is
the official one.

Run:
    uv run python scripts/experiments/run_bank.py --epsilon 1 --seed 0
"""
...
def main() -> int:
    ...
    train_csv = DATA_DIR / "bank_train.csv"
    if not train_csv.exists():
        print("Bank data not found. Run scripts/datasets/bank/preprocess.py first.", file=sys.stderr)
        return 2
    ...
    eps = args.epsilon
    exp_name = f"bank_eps{_eps_tag(eps)}_seed{args.seed}"
    exp_folder = REPO_ROOT / "results" / "raw" / "bank" / exp_name
    exp_folder.mkdir(parents=True, exist_ok=True)
    setup_logging(log_file=str(exp_folder / "log.txt"))

    meta = str(DATA_DIR / "bank_metadata.json")
    priv_data = TabularCSV(csv_path=str(train_csv), metadata_path=meta)
    priv_info = priv_data.get_tab_info()
    test_data = TabularCSV(csv_path=str(DATA_DIR / "bank_test.csv"), metadata_path=meta)
    ...
    record = {
        ...
        "kind": "bank_marketing",
        "official_script_path": "n/a (new dataset via scripts/experiments/run_bank.py)",
        "deviation": "new dataset (UCI Bank Marketing); official TabularEmbedding + algorithm; seeded.",
        "official_commit_sha": DPSDA_SHA,
        "command": f"python scripts/experiments/run_bank.py --epsilon {eps} --seed {args.seed}",
        ...
        "dataset_name": "Bank Marketing (real)",
        "dataset_source": "data/bank/ (see data/bank/manifest.json)",
